...
- A fully working, public mod that is uploaded directly through Discord, Google Drive, or GitHub.
...
This example comes directly from the first entry of `publicmoddata.json`.
...
After the two options is `install`, which provides the URL for downloading the mod. This depends on what provider the mod is being downloaded through.
...
- If your mod is a `.pak` file:
    1. Repeat the first two steps for the process above.
    2. Insert the edited name into both `installpath` and `deinstallpath`.

## Frontend JSON Code:

- `name`: Simply put your name of your Brick Rigs mod, in **plain text only**
- `image`: This is something that only BRMM's contributors can change.
