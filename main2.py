...
path = filedialog.askdirectory(title="Select your 'BrickRigs' folder.")
...
urls = ["https://github.com/anonymous-editor/BRMI/blob/main/M1.png", 
       ...
       "https://github.com/anonymous-editor/BRMM/blob/main/M8.png",
]
...
button = ctk.CTkButton(frame_3, command=lambda: download_file('1D6vJSi0rz6ix2oPLwFk9S2Bai2xOTGiS', f'{path}\\Mods\\BricksdaleSpeedway.zip'), **install_button())
...
button = ctk.CTkButton(frame_4, command=lambda: download_file('1q8GpP2iiuEkmNDHihcQnn-K7Y0HP_pUy', f'{path}\\Mods\\batt_showroom.zip'), **install_button())
button.pack(**install_button_packing())
...
button = ctk.CTkButton(frame_5, command=lambda: download_file('1z9TrMazaFOLkTJ8r4h5lWJ1lkLq6QzLr', f'{path}\\Mods\\FNAF1.zip'), **install_button())
button.pack(**install_button_packing())
...
button = ctk.CTkButton(frame_6, command=lambda: download_file('1RnQKGILY8f-v8Cq8Hpp-acye_bRsTE0v', f'{path}\\Mods\\VanillaContentExpansion.zip'), **install_button())
button.pack(**install_button_packing())
...
button = ctk.CTkButton(frame_7, command=lambda: download_zipfile('https://cdn.discordapp.com/attachments/751767065970475093/1181849071930060831/BlackTheme.zip?', f'{path}\\Mods\\BlackTheme.zip'), **install_button())
button.pack(**install_button_packing())
...
button = ctk.CTkButton(frame_8, command=lambda: download_file('1QvqNwaJJvweAr1eKNzNpVWseFEsj36iq', f'{path}\\Mods\\Airstrike.zip'), **install_button())
button.pack(**install_button_packing())
...
button = ctk.CTkButton(frame_9, command=lambda: download_file('1y5QEsYv1HCeNGna2O9RYtDmuNlTNdnzc', f'{path}\\Mods\\batt_knife.zip'), **install_button())
button.pack(**install_button_packing())
...
textbox_16 = ctk.CTkLabel(frame_10, text="Remastered Skies Mod", **mod_title_font())
textbox_16.pack(anchor="center", padx=10, pady=10)

image = Image.open(BytesIO(responses[7].content))
ctk_image = ctk.CTkImage(**create_ctk_image(image))
image_label = ctk.CTkLabel(frame_10, image=ctk_image, text="")
image_label.pack(**create_image_label_packing('center'))
...
button = ctk.CTkButton(frame_10, command=lambda: downloadpak_file('1SRmDYR2DLU2YQbCzs2ExBz_huj1GkLQv', f'{path}\\Content\\Paks\\APRS-WindowsNoEditor_P.pak'), **install_button())
button.pack(**install_button_packing())
